    ...
    *,
    ...
    if cached_apikey_data:
        ...
        context = RequestContext(
            ...
            api_key_timezone=cached.timezone or "UTC",
    ...
    if allowed_ids:
        ...
        matching_models = (
            await session.execute(
                select(LogicalModel)
                .where(
                    LogicalModel.name == logical_model_name,
                    LogicalModel.is_active,
                    LogicalModel.id.in_(allowed_ids),
                )
                .order_by(LogicalModel.id.asc())
            )
        ).scalars().all()
    ...
    context = RequestContext(
        ...
        payload=payload,
        stream=stream,
        request_logging_enabled=api_key.request_content_logging_enabled if api_key.request_content_logging_enabled is not None else settings.default_request_logging_enabled,
        response_logging_enabled=api_key.response_content_logging_enabled if api_key.response_content_logging_enabled is not None else settings.default_response_logging_enabled,
        api_key_id=api_key.id,
        api_key_name=api_key.name,
